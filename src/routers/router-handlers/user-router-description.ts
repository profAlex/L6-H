...
export const createNewUser = async (req: Request, res: Response) => {
    ...
    try{
        ...
        if(error instanceof CustomError) {
            ...
            res.status(HttpStatus.InternalServerError).json(JSON.stringify(error));
        }
    }
    ...
    res.status(HttpStatus.InternalServerError).send('Unknown error while attempting to create new user or couldn\'t return created user from Query Database.');
    return;
};
